...
export const cache = {
  get<T>(key: string): T | null {
    try {
      ...
      if (Date.now() > entry.expiry) { store.removeItem(PREFIX + key); return null; }
      return entry.data;
    } catch { return null; }
  },
  set<T>(key: string, data: T, ttl = DEFAULT_TTL): void {
    try {
      const store = safeStorage();
      if (!store) return;
      store.setItem(PREFIX + key, JSON.stringify({ data, expiry: Date.now() + ttl }));
    } catch { /* storage full */ }
  },
  del(key: string): void { try { safeStorage()?.removeItem(PREFIX + key); } catch { /**/ } },
  clear(): void {
    try {
      const store = safeStorage();
      if (!store) return;

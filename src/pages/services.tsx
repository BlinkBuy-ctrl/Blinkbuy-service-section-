...
export default function ServicesPage() {
  ...
  useEffect(() => {
    ...
    if (prev.search !== search || prev.category !== category || prev.location !== location ||
        prev.priceRange !== priceRange || prev.onlineOnly !== onlineOnly || prev.sortBy !== sortBy) {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        console.error(e);
        if (mounted) { setServices([]); setTotal(0); }
      } finally {
